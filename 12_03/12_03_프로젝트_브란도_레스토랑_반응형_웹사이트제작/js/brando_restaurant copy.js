...
/*             var secTop2 = $("#section02").offset().top-500;

            $(window).scroll(function(){
                $(this).scrollTop()>=secTop2
            }) */
            ...
            //스크롤 이벤트(패럴럭스)
            // var sec2Top = $("#section02").offset().top-500; //섹션2번의 탑 값에 부딪치게 되면 
            // var sec3Top = $("#section03").offset().top-500;
            // var sec4Top = $("#section04").offset().top-500;
            
            var secTop=[]; //위의 섹션 탑 값 변수를 배열로 저장(기억); 변수 많이 안 쓰려고

/*                 
                secTop[0]  = $("#section02").offset().top-500; //section태그 이용;#section02 = section.eq(1)
                secTop[1]  = $("#section03").offset().top-500; //sectiion.eq(2)
                secTop[2]  = $("#section04").offset().top-500; //sectiion.eq(3) 
                secTop[3]  = $("#section05").offset().top-500; //sectiion.eq(4)
                secTop[4]  = $("#section06").offset().top-500; //sectiion.eq(5)
                secTop[5]  = $("#section07").offset().top-500; //sectiion.eq(6)
                secTop[6]  = $("#section08").offset().top-500; //sectiion.eq(7)
                secTop[7]  = $("#section09").offset().top-500; //sectiion.eq(8)
                secTop[8]  = $("#section10").offset().top-500; //sectiion.eq(9)
                secTop[9]  = $("#section11").offset().top-500; //sectiion.eq(10)
                secTop[10] = $("#section12").offset().top-500; //sectiion.eq(11)
                secTop[11] = $("#section13").offset().top-500; //sectiion.eq(12)
                secTop[12] = $("#section14").offset().top-500; //sectiion.eq(13) 
*/

                var n = $("section").length-1; // 섹션의 총 갯수 14개 중에 섹션1(슬라이드섹션)빼고 13개 섹션만 적용
                //for(i=0;i<=12;i++){ // ☝ i에 섹션 갯수대로 넣기를 자동화 시키기
                for(i=0;i<=n-1;i++){ //변수 i는 배열의 순서 번호, 섹션은 13개지만 0부터 12까지 나와야돼서 n-1
                    secTop[i] = $("section").eq(i+1).offset().top-600;
                } 
                
                secTop[0] === $("section").eq(0+1);
               
               console.log( secTop );

                $(window).scroll(function(){
                        
                    if( $(this).scrollTop() > secTop[0] ){
                        $("#section02").addClass("addEvent");
                    }
                    else{
                        $("#section02").removeClass("addEvent");
                    }
                    if( $(this).scrollTop() > secTop[1] ){
                        $("#section03").addClass("addEvent");
                    }
                    else{
                        $("#section03").removeClass("addEvent");
                    }
                    if( $(this).scrollTop() > secTop[2] ){
                        $("#section04").addClass("addEvent");
                    }
                    else{
                        $("#section04").removeClass("addEvent");
                    }
                    if( $(this).scrollTop() > secTop[3] ){
