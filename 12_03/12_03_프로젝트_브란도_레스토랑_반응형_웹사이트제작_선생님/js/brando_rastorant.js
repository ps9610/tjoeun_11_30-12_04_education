            ...
            $('#submit').on({
                click:  function(event){  //전송 버튼 클릭 후 유효성 검사 시작
                   event.preventDefault(); //전송 버튼의 고유기능을 삭제
                    ...
                    var regExpName = /[^a-zA-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/; //입력 값 1글자 이상
                   ...
                   setId = setInterval(function(){
                        ...
                        if(cnt>=1){
                            ...
                            function formSubmitFn(){
                                ...
                                if( regExpName.test( $('#irum').val() ) === true || regExpMail.test( $('#mail').val() ) === false ||  regExpMessage.test( $('#message').val() ) === false ){

                                    //한글영문이 아닌거가 있다면
                                    if(   regExpName.test($('#irum').val()) === true ){
